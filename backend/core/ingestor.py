...
def ingest_file(filepath):
    ...
    if suffix == ".csv":
        events = _parse_csv_file(filepath)
        ...
        if content.startswith("["):
            ...
            for item in raw_list:
                event = _base_event(filepath.stem, json.dumps(item))
                ed = event["event_data"]
            ...
            for line in content.splitlines():
                line = line.strip()
                if line:
                    event = _parse_json_line(line, filepath.stem)
                    if event:
                        events.append(event)
        ...
        with open(filepath, encoding="utf-8") as f:
            for line in f:
                ...
                if not line:
                    continue
                json_event = _parse_json_line(line, filepath.stem)
                if json_event:
                    events.append(json_event)
                else:
                    events.append(_parse_syslog_line(line))

    out_path = PROCESSED_DIR / (filepath.stem + "_processed.json")
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(events, f, indent=2)

    print(f"Ingested {len(events)} events from {filepath.name} -> {out_path.name}")
    return events
